...
EXPOSURE_US = 1000   # IGNORED BY THIS SENSOR. Measured with probe_c:
                     # requested 250, 500, 1000, 2000, 4000 and 30000 us
                     # -- the applied value came back one frame period
                     # every single time. The setter is a no-op. The
                     # CALL still matters though: auto_exposure(False,
                     # ...) is what switches the AEC off. Leave it in.
                     # Without it the AEC sees the dark floor and opens
                     # the shutter to its 16648 us ceiling.
...
import time
print("=== scan_fit_fps_live.py -- VERSION: mini-junction-v13 ===")
                    ...
                    if r is not None:
                        ...
                        if g is not None:
                            ...
                            if plausible(X_mm):
                                ...
                                # moved further than physically possible
                                # since the last fix -- report Y only
