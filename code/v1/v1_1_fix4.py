...
EXPOSURE_US = 3500   # WORKS NOW -- fixed by the Sept development firmware
...
# ===================== BENCH MODE ==========================================
# BENCH = 1 turns this file into a benchmark of ITSELF and then stops.
#
# It captures BENCH_FRAMES frames once (hold everything still, on a
# junction), then times every stage of the pipeline on those same frames
# using the very same kernels the loop uses -- col_scan, row_scan, the
# fits, grad4, Stage 2, the overlay -- so the numbers are the real ones, not
# a re-implementation's. It then runs the full drive estimate and the full
# rotation estimate on every frame and reports the SPREAD of X, Y, T, phi:
# with nothing moving, every bit of spread is the pipeline's own noise.
#
# Three tables come out:
#   TIME      us per call per stage, and what fraction of a frame it is
#   NOISE     sd of each output over the static frames
#   VARIANTS  the two expensive stages at cheaper settings, same frames,
#             so you can see what each setting buys and what it costs
#
# It does not change anything. Set BENCH back to 0 to drive.
BENCH = 0
